...
draw_text_ext_transformed((xtxt + ((changex*dist) * textlayer))+ox ,(ytxt+2 + ((changey*dist) * textlayer))-3.8+oy, text,50,wide, xscale * xMULTI, yscale * yMULTI, 0);
}} else {draw_set_color(make_color_rgb(255/a,255/a,255/a))
	draw_text_ext_transformed((xtxt + ((changex*dist) * textlayer))+ox ,(ytxt+2 + ((changey*dist) * textlayer))-3.8+oy, text,50,wide, xscale * xMULTI, yscale * yMULTI, 0); 
}

if locked = 1 { draw_sprite(s_lock,0,xtxt,ytxt) }

linecolor = make_color_rgb((255/a),(255/a),(255 - (5.1 * col))/a)
