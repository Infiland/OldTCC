...
draw_text(camx+512,camy+230,"Read Rules before you do so! Otherwise mods will be very angry!")
...
} else { draw_text(camx+512,camy+200,"The level must have a player and a door placed in order to be able to publish\nit on Steam Workshop!") }
} else { draw_text(camx+512,camy+200,"Diamond Medal Time must be the same or higher than your level completion time.")  }
...
//} else { draw_text(camx+512,200,"A Thumbnail is missing, take a picture of the level.") }
} else {
if !instance_exists(o_playerspawner) || !instance_exists(o_door) {
draw_text(camx+512,camy+200,"The level must have a player and a door placed in order to be able to publish\nit on Steam Workshop!")
} else {
draw_text(camx+512,camy+200,"Complete the Level in order to be able to publish it on Steam Workshop!")
}
}
